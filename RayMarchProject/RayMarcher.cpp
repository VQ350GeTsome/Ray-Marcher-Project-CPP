...
{
	float amLight = light->getAmbientLight();		//Scene ambient light

	float res = 1.0f - amLight;						//Totally lit for now
...
std::vector<std::vector<vec3>> RayMarcher::marchScreen() 
{
	...
	for (int x = 0; x < width; x++) {
		for (int y = 0; y < height; y++)
		{
			...
			if (hitSDF != nullptr)										//If we hit an SDF (if it's not a nullptr)
			{															
